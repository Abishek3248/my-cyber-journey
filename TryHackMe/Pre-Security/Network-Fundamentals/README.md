...
- In this task, an interactive lab was provided where:
- Malicious traffic was shown in red packets.
- Legitimate traffic was shown in green packets.
- The target web server had the IP 203.0.110.1.
- The malicious traffic was using port 80 (HTTP).

### Objective
- Block malicious packets from reaching the web server 203.0.110.1 by creating a firewall rule.

### Approach
- dentified the source and destination IPs of the malicious packets.
- Created a firewall rule to block all traffic destined for the server’s port 80.
- Used the DROP action to silently discard packets instead of rejecting them.
- Rule was applied with conditions:
...
### Outcome
- All red (malicious) packets were successfully blocked.
- Green (legitimate) traffic continued to pass through without interruption.
